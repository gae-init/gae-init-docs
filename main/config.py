...
REFERENCE_DEF = {
    ...
    'utils': [
        ('order_by_link.md', "(property, title, ignore=['cursor'])"),
        ('filter_by_link.md', "(property, value, icon=None, ignore=['cursor'])"),
        ('next_link.md', "(more_url, caption='Next Page')"),
        ('prefetch_link.md', '(url)'),
        ('auth_icon.md', '(auth_id)'),
        ('auth_icons.md', '(auth_ids)'),
        ('html_element.md', '(name, content)'),
      ],
    'forms': [
